...
def dplr(
    ...
    # Initialize B
    if B_random:
        print("'B_random' is deprecated in favor of B_init='random' and will be deprecated in a future version.")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.verbose and len(kwargs) > 0:
            print.info(f"{type(self)} extra kwargs: {kwargs}")
    ...
    def __init__(
        ...
        **init_args,
        ...
        if measure is not None:
            print("Warning: 'measure' option changed to 'init' and will be removed in a future version.")
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def _setup_C(self, L):
        ...
        if self.l_kernel.item() == 0:
            ...
            if self.verbose: print(f"S4: Doubling length from L = {self.l_kernel.item()} to {2*self.l_kernel.item()}")
            double_length = True
    ...
    def register_params(self, A, B, C, inv_dt, P):
        ...
        if self.verbose:
            print(f"Constructing S4 (H, N, L) = ({self.H}, {self.N}, {self.l_max})")
